...
The LVGL cache framework implements a Least Recently Used (LRU) cache strategy, which is a widely used cache eviction
algorithm. When the cache reaches its capacity limit, the LRU strategy evicts entries that have not been accessed for
the longest time to make space for new entries.
...
2. **Doubly Linked List**: Used to maintain the usage order of cache entries, with recently used entries at the head of
   the list and least recently used entries at the tail.
...
The count-based LRU cache (`lv_cache_class_lru_rb_count`) limits the number of entries in the cache. When the number of
entries in the cache reaches the maximum value, it evicts the least recently used entries.
...
The size-based LRU cache (`lv_cache_class_lru_rb_size`) limits the total memory size occupied by the cache. When the
cache memory usage reaches the maximum value, it evicts the least recently used entries until there is enough space to
...
Cache entries use a reference counting mechanism to track how many clients are currently using the entry. This mechanism
ensures that even if an entry is evicted from the cache, resources in use will not be freed until all clients release
them.
...
The cache framework uses LVGL's memory allocation functions `lv_malloc` and `lv_free` to manage memory. These functions
can use different memory allocation strategies depending on the platform and configuration.
...
The cache framework uses a reference counting mechanism to manage memory release. A cache entry's memory is only freed
when its reference count drops to zero and it is marked as invalid.
...
The cache framework uses LVGL's assertion mechanism to check the validity of parameters and states, helping developers

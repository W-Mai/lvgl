# LVGL Cache Framework API Reference
...
- `cache_class`: Pointer to the cache class that defines the behavior of the cache. Currently only support one two
  builtin classes:
...
- This code initializes an image cache with a maximum size of `size` bytes. The cache uses the LRU-based cache with
  size-based eviction policy. The cache entry node size is `sizeof(lv_image_cache_data_t)`. The cache compares cache
  entries using the `image_cache_compare_cb` function. The cache frees cache entries using the `image_cache_free_cb`
  function.
...
`NULL`.
If the entry is found, it's priority will be changed by the cache's policy. And the `lv_cache_entry_t::ref_cnt` will be
incremented.
...
- This code tries to get the decoded image from the cache using the `lv_cache_acquire` function. If the cache hit, it
  returns the decoded image. Adn it will increment the `lv_cache_entry_t::ref_cnt` of the entry. If the cache miss, it
...
- The cache framework will free the memory of the data pointer.
- The user should not free the memory of the data pointer in this callback.
...
#### lv_image_cache_deinit

```c
void lv_image_cache_deinit(void);
```

**Description**: Deinitializes the image cache and frees all associated resources.
...
#### lv_image_header_cache_deinit

```c
void lv_image_header_cache_deinit(void);
```

**Description**: Deinitializes the image header cache and frees all associated resources.

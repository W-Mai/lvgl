...
static lv_span_coords_t make_span_coords(lv_span_t * prev_span, lv_span_t * curr_span, int32_t width);
...
lv_span_coords_t lv_spangroup_get_span_coords(lv_obj_t * obj, lv_span_t * span)
{
    ...
    return make_span_coords(prev_span, curr_span, width);
}
...
lv_span_t * lv_spangroup_get_span_by_point(lv_obj_t * obj, lv_point_t * point)
{
    ...
    lv_span_t * curr_span;
    LV_LL_READ(spans, curr_span) {
        lv_span_coords_t coords = make_span_coords(prev_span, curr_span, width);
...
static void lv_spangroup_event(const lv_obj_class_t * class_p, lv_event_t * e)
{
    ...
    if(code == LV_EVENT_DRAW_MAIN) {
        ...
        for(uint32_t i = 0; i < lv_array_size(&spans->event_spans) ; i++) {
            ...
            if(lv_area_is_point_on(&coords.heading,  &rel_point, 0) ||
               lv_area_is_point_on(&coords.middle,   &rel_point, 0) ||
               lv_area_is_point_on(&coords.trailing, &rel_point, 0)) {
                hit_span_pair = pair;
                break;
            }
        }

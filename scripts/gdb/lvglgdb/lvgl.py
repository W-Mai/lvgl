...
class LVList(Value):
    ...
    def _next(self, node):
        return Value(int(node) + self._next_offset).cast(self.lv_ll_node_t, ptr=True).dereference()

    def _prev(self, node):
        return Value(int(node) + self._prev_offset).cast(self.lv_ll_node_t, ptr=True).dereference()
...
def set_lvgl_instance(lv_global: Optional[Value]):
